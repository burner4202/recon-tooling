<?php

namespace Vanguard\Console\Commands\Coordination;

use Illuminate\Console\Command;

use GuzzleHttp\Client;
use GuzzleHttp\Psr7;
use GuzzleHttp\Exception\RequestException;

use Vanguard\WatchedSystems;


class getWatchedSystemsCommand extends Command
{
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'watched:systems';

    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Gets all the data from the watched systems.';

    /**
     * Create a new command instance.
     *
     * @return void
     */
    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Execute the console command.
     *
     * @return mixed
     */
    public function handle()
    {


        # Make an API Call.
        # What systems are being watched.

        $watched_systems = WatchedSystems::orderBy('solar_system_name')->get();

        # We have systems.
        if($watched_systems) {

            # Cycle Each System & Update
            foreach($watched_systems as $systems) {

             $system = $this->get_system_local($systems->solar_system_id);

             $alliance_array = array();

             if($system) {

                $adash_url = key($system[0]);

                foreach($system[0] as $local_scan) {

                    $total = $local_scan[0]->total;
                    $alliances = $local_scan[1]->alliances;

                    if($alliances) {

                        foreach($alliances as $alliance) {

                            $alliance_id = key($alliance);

                            $no_of_alliances = $alliance->$alliance_id;
                            $alliance_array[] = $no_of_alliances;


                        }
                    }

                }

                $alliance_string = implode(', ', $alliance_array);

                $this->info('https://adashboard.info/intel/local/view/'.$adash_url);
                $this->info($total);
                $this->info($alliance_string);

                $adash = 'https://adashboard.info/intel/local/view/'.$adash_url;

                $update = WatchedSystems::updateOrCreate([
                    'solar_system_id'                             => $systems->solar_system_id,
                ],[
                    'solar_system_name'                           => $systems->solar_system_name,
                    'constellation_id'                            => $systems->constellation_id,
                    'constellation_name'                          => $systems->constellation_name,
                    'region_id'                                   => $systems->region_id,
                    'region_name'                                 => $systems->region_name,
                    'adash_url'                                   => $adash,
                    'local_numbers'                               => $total,
                    'local_alliances'                             => $alliance_string,

                ]);   


            }


            # END FOREACH


        }

        # END IF
    }

}






public function get_system_local($system_id) {

    /*
    /recon/api/ds/recentfromsystem/<system id> shows the dscans in that system (last minute), ordered by most recent, limited to about 6 i think, output is json
    /recon/api/lo/recentfromsystem/<system id> for local scans
    oh some details: dscans only show 25 ships if more than 5 on dscan (as not to clutter you too much with random ibusses and frigates)
    */

    # Grab our API key from ENV.
    $secret_key = config('adash.secret_key');

    try {
        $client = new Client([
            'headers' => ['aD_APISecret' => $secret_key]
        ]);

        $response = $client->request(
            'GET',
            'https://adashboard.info/recon/api/lo/recentfromsystem/' . $system_id,
            ['aD_APISecret' => $secret_key]
        );

        return json_decode($response->getBody()->getContents());

    } catch (RequestException $e) {
            //echo Psr7\str($e->getRequest());
        if ($e->hasResponse()) {
            $this->info($e);
        }
    }


}

public function get_system_dscan($system_id) {

    /*
    /recon/api/ds/recentfromsystem/<system id> shows the dscans in that system (last minute), ordered by most recent, limited to about 6 i think, output is json
    /recon/api/lo/recentfromsystem/<system id> for local scans
    oh some details: dscans only show 25 ships if more than 5 on dscan (as not to clutter you too much with random ibusses and frigates)
    */

    # Grab our API key from ENV.
    $secret_key = config('adash.secret_key');

    try {
        $client = new Client([
            'headers' => ['aD_APISecret' => $secret_key]
        ]);

        $response = $client->request(
            'GET',
            'https://adashboard.info/recon/api/ds/recentfromsystem/' . $system_id,
            ['aD_APISecret' => $secret_key]
        );

        return json_decode($response->getBody()->getContents());

    } catch (RequestException $e) {
            //echo Psr7\str($e->getRequest());
        if ($e->hasResponse()) {
            $this->info($e);
        }
    }


}
}
