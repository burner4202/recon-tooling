...
class SocialAuthController extends Controller
{
    ...
    public function redirectToProvider($provider)
    {
    	...
    	// This requires GICE permissions to allow groups to be automatically allocated upon logging in. // Talk to Bittey / Kilgarth before enabling.
        //->scopes(['groups'])
        //->scopes(['openid', 'groups-limited'])  # Moved to provider - Got access to scopes groups-limited 18/2/20 # Bittey.
        ->scopes(['openid'])
    	->redirect();
    }
    ...
    public function handleProviderCallback($provider)
    {
    	...
    	$user = $this->users->findBySocialId($provider, $socialUser->sub);
    	...
    	if (! $user) {
            ...
            // Only allow missing email from GICE provider
           //if (! $socialUser->getEmail()) {
            //    return strtolower($provider) == 'gice'
     ...
     */
    public function getGICEEmail()
    {
    	$account = $this->getSocialAccountFromSession();

    	return view('auth.social.gice-email', compact('account'));
    }
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function postGICEEmail(SaveEmailRequest $request)
    {
    	...
    	$user = $this->socialManager->associate($account, 'gice');
    ...
    private function handleMissingEmail($socialUser)
    {
    	...
    	return redirect()->to('auth/gice/email');
    }
