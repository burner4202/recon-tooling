			...
			<div class="table-responsive top-border-table" id="location-table-wrapper">
				...
				<table class="table" id="moons">
					<thead>
						...
						<th> @sortablelink('moon_value_56_day', 'Value (56 Days)')</th>
					...
					<tbody>
						...
						<tr>
							...
							@endif
							<td style="vertical-align: middle"><a target="_blank" href="{{ route('solar.system', $moon->moon_system_id)}}">{!! $moon->moon_system_name !!}</a></td>
							...
							<td style="vertical-align: middle"><a target="_blank" href="{{  route('structures.view', $moon->str_structure_id_md5 )}}">{{ $moon->str_name }}</a></td>
							@if($moon->str_type_id < 1)
							<td></td>
							@else
							<td style="vertical-align: middle"><img class="img-circle" src="https://images.evetech.net/types/{{ $moon->str_type_id }}/render?size=32">&nbsp;{{ $moon->str_type }}</td>
							@endif
							@if($moon->str_owner_corporation_id > 1)
							<td style="vertical-align: middle"><a href="{{ route('corporation.view', $moon->str_owner_corporation_id )}}"><img class="img-circle" src="https://images.evetech.net/corporations/{{ $moon->str_owner_corporation_id }}/logo?size=32">&nbsp;{{ $moon->str_owner_corporation_name }}</a></td>
							@else
							<td></td>
							@endif
							@if($moon->str_owner_alliance_id > 1)
							<td style="vertical-align: middle"><a href="{{ route('alliance.view', $moon->str_owner_alliance_id )}}"><img class="img-circle" src="https://images.evetech.net/alliances/{{ $moon->str_owner_alliance_id }}/logo?size=32">&nbsp;{{ $moon->str_owner_alliance_name }} ({{ $moon->str_owner_alliance_ticker }})</a></td>
							@else
							<td></td>
							@endif
							@permission('structure.hitlist')
							<td class="text-center" style="vertical-align: middle">
								@if($moon->str_type_id > 1)
								@if($moon->str_hitlist == 1)

								<a href="{{ route('structures.hitlist_remove', $moon->str_structure_id_md5) }}" class="btn btn-danger" id="hitlist_remove" data-toggle="tooltip" data-placement="right" title="Remove from the Hitlist">
									<i class="glyphicon glyphicon-minus"></i>
								</a>

								@else

								<a href="{{ route('structures.hitlist_add', $moon->str_structure_id_md5) }}" class="btn btn-danger" id="hitlist_add" data-toggle="tooltip" data-placement="right" title="Add to Hitlist">
