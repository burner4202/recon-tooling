...
class UsersController extends Controller
{
    ...
    public function view(User $user, ActivityRepository $activities)
    {
    	...
    	$characters = ESITokens::where('esi_user_id', $user->id)
    	->orderBY('esi_name')
    	->get();

      	// Load appropriate view for displaying the characters

    	return view('user.view', compact('user', 'userActivities', 'characters'));
    }
    ...
    /**
     * Display the ledger balance for the user_id
     *
     * @return Response
    */
    public function userLedgerBalance($user_id)
    {

        $balance = DB::table('ledger')
        ->where('user_id', $user_id)
        ->select(DB::raw('sum(quantity) as quantity'), DB::raw('sum(value) as total'), DB::raw('sum(tax) as tax'))
    ...
    /**
     * Display the payment balance for the user_id
     *
     * @return Response
    */
    public function userPaymentBalance($user_id)
    {

        $paymentsToDate = DB::table('tax_mans_wallet')
        ->where('user_id', $user_id)
     ...
     * @return Response
    */
    public function userLastPayment($user_id)
    {

        $lastPayment = DB::table('tax_mans_wallet')
        ->where('user_id', $user_id)
        ->orderBy('date', 'desc')
        ->first();
